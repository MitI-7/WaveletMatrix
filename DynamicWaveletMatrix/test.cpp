#include<bits/stdc++.h>
#include "../dynamicWaveletMatrix/DynamicWaveletMatrix.hpp"
...
double speed_access(uint64_t num, uint64_t num_of_alphabet) {
    ...
    DynamicWaveletMatrix wm(num_of_alphabet, data);
...
double speed_erase(uint64_t num, uint64_t num_of_alphabet) {
    ...
    DynamicWaveletMatrix dwm(num_of_alphabet, data);
...
double speed_insert(uint64_t num, uint64_t num_of_alphabet) {
    ...
    DynamicWaveletMatrix dwm(num_of_alphabet);
    auto start = std::chrono::system_clock::now();
    for (int i = 0; i < num; ++i) {
...
bool same(DynamicWaveletMatrix expected, DynamicWaveletMatrix actual) {
    ...
    for (int i = 0; i < expected.bit_arrays.size(); ++i) {
        ...
        for (int j = 0; j < expected.bit_arrays.at(i).size; ++j) {
            expected_bits.emplace_back(expected.bit_arrays.at(i).access(j));
        }
        for (int j = 0; j < actual.bit_arrays.at(i).size; ++j) {
            actual_bits.emplace_back(actual.bit_arrays.at(i).access(j));
        }
...
bool test_erase(uint64_t num, uint64_t num_of_alphabet) {
    ...
    DynamicWaveletMatrix actual(num_of_alphabet, data);

    for (int i = 0; i < num; ++i) {
        uint64_t pos = i == 0 ? 0 : randxor() % data.size();
        data.erase(data.begin() + pos);

        DynamicWaveletMatrix expected(num_of_alphabet, data);
        actual.erase(pos);
...
bool test_insert(uint64_t num, uint64_t num_of_alphabet) {
    ...
    DynamicWaveletMatrix actual(num_of_alphabet);
    vector<uint64_t> data;
    for (int i = 0; i < num; ++i) {
